...
**Last updated:** 2026-09-06 (contributor README + app-wide Groww-style dark theme)
...
- [x] 2026-09-05 — Login UI rebuilt Groww-style + forgot-password flow (Claude wrote it at the user's request): brand tokens in `index.css`, split-screen `LoginPage` (sign-in/sign-up/forgot), new `ResetPasswordPage` + `/reset-password` route before the session gate, shared `BrandLogo`, sidebar active pill green. Build + lint green. **Remaining: add `{origin}/reset-password` to Supabase Auth → Redirect URLs, then browser-verify the email round-trip**
...
| 2026-09-05 | Login page rebuilt Groww-style (user: "improve the ui/ux … look like groww.com"): `--color-brand-*` tokens (Groww `#00b386`) in `index.css` `@theme`; `primaryButtonClass`/`inputClass` re-skinned so every form app-wide shares the green; `BrandLogo` shared by login + sidebar; sidebar active pill light-green; split-screen layout with a decorative brand panel (hero.png rejected — purple, off-brand). Contrast call: white on Groww-green is ~2.7:1 — kept only on the large CTA for brand fidelity; small green text uses `brand-700` (AA) | User's explicit ask; Claude implemented |
| 2026-09-05 | Forgot password implemented — Supabase Auth only, zero backend change: `resetPasswordForEmail` (redirect `{origin}/reset-password`) → PKCE code exchange → `updateUser({ password })`. Recovery is detected by PATHNAME, not the `PASSWORD_RECOVERY` auth event (with PKCE that event fires unreliably — often only `SIGNED_IN`), so `/reset-password` is matched BEFORE the session gate in `App.tsx`; a stuck exchange falls back to "link expired" after 8s. `/login` is now a real route; all other pre-auth paths redirect to it. **Prerequisite: add `{origin}/reset-password` to Supabase Auth → URL Configuration → Redirect URLs** | User asked to "implement forgot password feature"; docs updated (frontend.md §4) |
| 2026-09-05 | **Backend deployed on Render (free tier): `https://project-1-jfr5.onrender.com`** — `/actuator/health` verified UP (includes DB check). Frontend `VITE_API_BASE_URL` now points at it (`frontend/.env`); local-dev fallback kept as a comment. Free-tier behavior: instance spins down when idle → **first request after idle takes ~50s**, so treat slow first loads as cold starts, not failures. CORS: Render's `CORS_ALLOWED_ORIGINS` must include each frontend origin (localhost:5173 is the baked-in default; add the deployed frontend origin when frontend hosting lands) | User deployed on their own ("backend is deployed in …onrender.com use this wherever needed"); Azure App Service path abandoned — see next row |
| 2026-09-05 | Azure App Service attempt (RG `finance`, app `fin-backend`, plan `ASP-studentsres-a282`): jar deployed via OneDeploy; root causes found — (1) portal created the app on the **Java 17** runtime while the app needs 21 (fixed via `az webapp config set --linux-fx-version "JAVA|21-java21"`); (2) the crash-loop burned the **Free F1 60 CPU-min/day quota** → site auto-disabled (`state: QuotaExceeded`) until 00:00 UTC. B1 (~$13/mo, billed even when stopped) was offered; user pivoted to Render instead. Same day: RG `finance` deleted at the user's request (fin-backend + oidc-msi identity) and the Azure GitHub workflow removed from the repo — **Azure fully cleaned up; Render is the only backend host** | Debugging journey; kept so a future session doesn't rediscover it |
| 2026-09-05 | **Frontend deployed on Vercel: `https://fin-track-three-tawny.vercel.app`**. Vite inlines `VITE_*` at BUILD time, so Vercel must define `VITE_SUPABASE_URL` / `VITE_SUPABASE_ANON_KEY` / `VITE_API_BASE_URL` and then **redeploy** (first build shipped without them → "Missing VITE_SUPABASE_URL" crash; verified 0 Supabase refs in the deployed bundle). Two more prod requirements found by probing: (1) Render `CORS_ALLOWED_ORIGINS` must list the Vercel origin (preflight returned `403 Invalid CORS request`) — comma-separated, keep `http://localhost:5173` for local dev; (2) SPA rewrites: added `frontend/vercel.json` (all paths → index.html) so the `/reset-password` email link deep-link works. Supabase Auth → Redirect URLs must include the Vercel origin too | User: "frontend url change it where ever needed" |
| 2026-09-05 | Gotcha: local (IntelliJ) + Render both hitting the same free Supabase exhausted its **15-client session-pooler cap** (`EMAXCONNSESSION`, real cause hiding as the LAST `Caused by:` under "Unable to start web server"). Fix: Hikari pool made configurable in application.yml — `maximum-pool-size: ${DB_POOL_MAX:5}`, `minimum-idle: 2`. Render sets `DB_POOL_MAX=8` (8+5=13<15, 2 spare); local uses the default 5. Rule: only ONE local instance at a time (two local runs = 5+5+8=18 > 15) | Surfaced when local boot failed after the Render deploy |
| 2026-09-06 | **App-wide dark theme (Groww-style) + toggle** (user: "implement Dark theme toggle all over the website … neon green and blue … same as groww"). Class-based `dark` variant in Tailwind v4 (`index.css` `@custom-variant`); `ThemeContext` (localStorage `ft-theme`, OS default) + pre-paint script in `index.html` (no FOUC — keep in sync). Income/expense are mode-flipping CSS vars exposed via `@theme inline` (`text-income`/`text-expense`; dark: neon green `#00e09e` / blue `#4da3ff`); surfaces are slate-950/900/800 `dark:` twins; charts read `lib/chartTheme.ts` via `useTheme()` (Recharts needs JS props). Toggle lives in the sidebar footer, login/reset headers and Settings → Appearance. Rose stays the danger color (delete/errors) — only expense *amounts* turn blue. Spec: frontend.md §25 | User's explicit ask; Claude implemented |
| 2026-09-06 | Root `README.md` rewritten for contributors (was stock Vite boilerplate): features, stack, architecture, setup for frontend+backend+Supabase, dark-theme notes, docs map, contributing conventions. `frontend/README.md` replaced with real dev notes (code map, conventions, Vercel deploy) | User: "add Readme.md file about the website to give it to an contributor" |
| 2026-09-06 | Dark-theme expense color revised blue → **red** (rose-400 `#fb7185` in `.dark --expense` + `chartTheme`) after seeing it live: "i want expense should be red and the income as it is in green … transfer in blue and loan in yellow is ok". Income (neon green), transfer (blue), loan (amber) unchanged; danger and expense share the red family, same as light mode | User review of the shipped theme |
